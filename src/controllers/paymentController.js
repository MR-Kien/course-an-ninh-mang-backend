...
//     if (!paymentId) {
//        console.error("❌ Không thấy paymentId trong extraData trả về từ MoMo Query");
//        return res.status(500).json({ message: "Lỗi xử lý (Không thấy paymentId)" });
//     }
...
//     if (!payment) {
//       await pool.query("ROLLBACK");
//       console.warn("⚠️ Không tìm thấy giao dịch 'pending' cho:", paymentId, "User:", userId);
...
//       await pool.query("ROLLBACK");
//       console.error("❌ SAI SỐ TIỀN:", payment.so_tien, "vs MoMo:", momoResponse.amount);
...
//     res.status(500).json({ message: "Lỗi server khi xác nhận", error: error.message });
//   }
// };
import pool from "../config/db.js";
import crypto from "crypto";
